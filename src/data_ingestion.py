        ...
        for line in lines:
            ...
            if data:
                ...
                if not existing_record:
                    ...
                    logging.warning(
                        f"Duplicate record {str(existing_record)} exists for file {file_path}"
                    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Argumennts to ingest weather data into sqlite3 database."
    )

    parser.add_argument(
        "-data", "--data_path", type=str, required=True, help="Path of text files."
    )
    parser.add_argument(
    ...
    # Configure logger.
    logging.basicConfig(
        filename="weather_data_ingestion.log",
        level=logging.INFO,
    ...
    if not os.path.exists(data_path):
        logging.error(
            f"Provided data path {data_path} don't exist. Please provide correct path."
        )
        sys.exit(0)
